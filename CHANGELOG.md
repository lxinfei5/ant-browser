...
- 内置字体插件：启动时自动同步并加载本地「强制字体（Monaco + 95% 微软雅黑）」插件，按嵌入文件做完整性校验，不允许商店覆盖或删除。官方 Chrome 137+ 改为调试口 `Extensions.loadUnpacked` 装入；不再使用 `--disable-extensions-except`，避免插件只写入配置却不启用。
