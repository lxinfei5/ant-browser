...
func (s *LaunchServer) buildMux() *http.ServeMux {
	...
	mux.HandleFunc("/api/v1/pool/accounts/batch-import", s.handleAccountBatchImport)
	mux.HandleFunc("/api/v1/pool/accounts/", s.handleAccountByID)
	mux.HandleFunc("/api/v1/pool/lease", s.handleLease)
	mux.HandleFunc("/api/v1/pool/lease/", s.handleLeaseByID)
	mux.HandleFunc("/api/v1/pool/proxies/", s.handlePoolProxyByID)
