...
type AccountDAO interface {
	...
	// UpdateAccountStatus 仅更新账号状态、冷却到期时间与 last_used_at（供代理失败冷却在事务内调用）。
...
const accountColumns = `
	account_id, account_name, account_ref,
...
// List 查询未删除的账号，支持 status / group_id 过滤
...
func (d *SQLiteAccountDAO) Upsert(account *Account) error {
	...
	_, err := d.db.Exec(`
		...
		account.AccountID, account.AccountName, account.AccountRef, account.Email, account.Phone,
...
func scanAccount(s scanner) (*Account, error) {
	...
	err := s.Scan(
		&a.AccountID, &a.AccountName, &a.AccountRef, &a.Email, &a.Phone,
...
// sqlRunner 抽象 *sql.DB 与 *sql.Tx 共同实现的执行接口，使 DAO 方法既可走事务也可走单连接。
