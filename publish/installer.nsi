...
!define PRODUCT_NAME    "ProfilePool"
!define PRODUCT_EXE     "profilepool.exe"
!define PRODUCT_ICON    "ProfilePool.ico"
!define UNINSTALL_KEY   "Software\Microsoft\Windows\CurrentVersion\Uninstall\ProfilePool"
!define INSTALL_DIR     "$PROGRAMFILES64\ProfilePool"
...
!macro WriteCloseProcessScript HANDLE
  ...
  FileWrite ${HANDLE} "function Get-ProfilePoolProcesses {$\r$\n"
  ...
  FileWrite ${HANDLE} "  $$procs = Get-ProfilePoolProcesses$\r$\n"
  ...
  FileWrite ${HANDLE} "$$left = Get-ProfilePoolProcesses$\r$\n"
...
OutFile "..\publish\output\ProfilePool-Setup-${VERSION}.exe"
...
!define MUI_DIRECTORYPAGE_TEXT_TOP "请选择 ProfilePool 的安装目录。建议安装到非 C 盘的可写目录，例如 D:\software\ProfilePool 或 E:\software\ProfilePool；不要安装到 C 盘、Program Files 或其他受权限保护的目录。"
...
!define MUI_FINISHPAGE_RUN_TEXT "Launch ProfilePool"
...
Section "ProfilePool (required)" SecMain
  SectionIn RO
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SecMain}    "ProfilePool main program and default config (required)"
